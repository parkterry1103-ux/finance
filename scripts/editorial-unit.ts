...
assert(registryResult.publishedCount === 1, 'Phase 5B Published 콘텐츠 수가 1개가 아닙니다.');
...
assert(publishedEditorialSummaryIndex.length === 1 && publishedEditorialSummaryIndex.every((item) => item.status === 'published'), '홈 요약 index에 draft가 포함됐습니다.');

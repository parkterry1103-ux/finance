...
- 단순 밈주, 근거 없는 급등락, 출처가 약한 소문성 이슈는 제외합니다.
- 실적, 공시, 산업 흐름, 정책, 수주, 제품, 고객사 변화처럼 가격 움직임을 설명할 수 있는 명확한 이슈가 있는 기업만 선정합니다.
- 매수/매도 추천이 아니라 `왜 움직였는지`, `어떤 흐름과 연결되는지`, `무엇을 확인해야 하는지`를 설명합니다.
- 직접 고객, 독점 공급, 매출 비중은 공시·IR·계약 원문으로 확인되지 않으면 단정하지 않습니다.
- 이미 같은 이슈로 다룬 Pick은 새 정보가 있을 때만 업데이트하고, 중복 콘텐츠는 피합니다.

### Pick 작성 구조

Pick 하나는 아래 순서로 작성합니다. 화면에는 핵심만 먼저 보이게 하고, 원문·세부 근거는 상세 또는 접힘 영역에서 확인하게 합니다.

1. 한 줄 제목: 급등락의 핵심 이유를 짧게 씁니다.
2. 왜 움직였나: 실적, 공시, 정책, 수주, 제품, 고객사 변화 중 실제 이슈를 설명합니다.
3. 어떤 시장 흐름과 연결되나: AI 반도체, 전력 인프라, 메모리, 파운드리처럼 더 큰 흐름으로 연결합니다.
4. 밸류체인에서 어느 단계인가: 최종 수요, 칩 설계, 메모리, 파운드리, 장비, 전력·냉각처럼 위치를 정합니다.
5. 같이 볼 기업 3개: 같은 흐름에서 먼저 비교할 대표 기업만 고릅니다.
6. 먼저 볼 숫자 3개: 매출, 영업이익률, 영업현금흐름, 재고, 매출채권, CAPEX처럼 이슈와 직접 연결되는 숫자만 고릅니다.
7. 좋은 신호: 다음 공시나 뉴스에서 확인할 긍정 조건을 씁니다.
8. 조심할 신호: 기대가 꺾일 수 있는 조건을 씁니다.
9. 원문/출처 링크: DART, SEC, 회사 IR, 공식 보도자료, 신뢰 가능한 뉴스 원문을 붙입니다.
10. 투자 추천 아님 문구: 목표주가나 매매 판단이 아니라 확인 포인트임을 분명히 씁니다.

### 초보자용 작성 원칙

- 어려운 용어는 처음 나올 때 한 문장으로 풀어씁니다.
- 숫자는 3개 이하만 먼저 보여주고, 나머지는 고급 참고자료로 넘깁니다.
- 목표주가, 예상 수익률, 매수/매도 타이밍 예측은 쓰지 않습니다.
- 결론은 `살까 말까`가 아니라 `무엇을 확인할까`로 마무리합니다.
- 뉴스 하나를 기업 하나로 끝내지 않고, 시장 흐름과 밸류체인으로 연결합니다.
- `확정 수혜`, `급등 예상`, `무조건`, `보장`, `저평가 확실`처럼 투자 추천처럼 보이는 표현은 피합니다.
- 불확실한 데이터는 `확인 필요`, `원문 연결 필요`, `공식 공시 기준 확인 필요`로 표시합니다.

### 웹사이트 반영 체크리스트

- 관련 기업이 기업 해설 페이지에 있는지 확인합니다.
- 관련 시장 흐름 지도와 연결되는지 확인합니다.
- 재무 쉽게 보기에서 핵심 숫자 3개가 표시되는지 확인합니다.
- 원문 출처가 있는지 확인합니다.
- 고급 참고자료는 기본 노출하지 않고 접힘 영역으로 유지합니다.
- 같이 볼 기업은 3개를 먼저 보여주고, 많은 관계는 시장 흐름 지도에서 확인하게 합니다.
- 새 Pick이 홈과 Pick 목록에 노출되어도 앱 동선이 `홈 -> Pick -> 시장 흐름 지도 -> 기업 해설 -> 재무 쉽게 보기`를 벗어나지 않는지 확인합니다.

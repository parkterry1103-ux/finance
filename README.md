...
- 첫 화면에는 브랜드 카피, `이번 주 Pick 보기`, `AI 반도체 흐름 보기` CTA만 강하게 둡니다.
- 홈에는 최신 Pick 1~3개, AI 반도체 & 데이터센터 흐름 미리보기, 초보자 가이드 3개 카드만 우선 노출합니다.
- 시장 흐름 미리보기는 `AI 수요 -> AI 칩 -> HBM -> 파운드리 -> 장비/전력`처럼 5단계 안팎으로 압축합니다.
- 13F, Form4, 기관 동향, 공시 전체 목록, 가격 상세, 공급망 참고, 관계 출처 전체는 홈에서 숨기고 기업 해설의 고급 참고자료로 보냅니다.
...
이 대표 섹터의 기본 흐름은 `AI 서버 수요 -> AI 칩/GPU -> HBM/메모리 -> 파운드리 -> 장비/소재/후공정 -> 서버/네트워크 -> 전력·냉각`입니다. 기본 화면은 Microsoft, Google, Amazon, NVIDIA, AMD, Broadcom, SK하이닉스, 삼성전자, Micron, TSMC, ASML, 한미반도체, Super Micro, Dell, Vertiv처럼 흐름을 이해하는 데 필요한 핵심 기업 10~15개만 먼저 보여줍니다.

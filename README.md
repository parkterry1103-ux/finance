...
기업의 사업과 재무 흐름, 금리·환율·원자재의 거시경제 흐름을 각각 살펴보고 서로 연결해 설명하는 리서치 웹사이트입니다.

Phase 5I 기업 판단의 모델과 freshness 규칙은 [docs/company-judgment-model.md](docs/company-judgment-model.md), 구현·공식 기준은 [docs/site-restructure-phase-5i.md](docs/site-restructure-phase-5i.md), 브라우저용 진행 기록은 [docs/plans/phase-5i-company-judgment-plan.html](docs/plans/phase-5i-company-judgment-plan.html)에서 확인할 수 있습니다.

## 사이트 재개편 방향

주가해부실은 기업 분석과 거시경제라는 두 축으로 구성됩니다.

기업 분석은 기업의 사업과 재무 흐름을 살펴보는 영역입니다.
거시경제는 금리·환율·원자재가 시장에 이어지는 흐름을 살펴보는 영역입니다.
...
- 1단계 완료 — 홈과 공통 내비게이션을 `기업 분석`, `거시경제` 두 진입점으로 단순화했습니다.
- 2단계 완료 — `/ko/companies`를 현재 지원 기업의 이름·티커·종목코드·별칭을 찾는 접근 가능한 로컬 검색 화면으로 개편했습니다.
- 3단계 완료 — 8개 기업 상세 화면을 근거가 연결된 핵심 판단, 기업별 핵심 숫자, 비교 가능한 차트, 중요한 변화와 주요 거시 변수 중심의 간결한 대시보드로 정리했습니다.
- 4A단계 완료 — 8개 기업 가치평가 준비도 감사와 공시 정규화, 결정론적 FCFF DCF·시나리오·민감도·역산 DCF 엔진을 구축했습니다.
- 4B단계 완료 — NVIDIA·Meta 두 기업의 작성 시점 맥락과 근거를 연결한 웹 리서치 리포트를 공개했습니다.
- 4C단계 완료 — 4A 엔진을 재사용한 5만 회 Monte Carlo 정적 실행으로 P50·P25–P75·P10–P90 범위, 입력 영향도와 장기 구간 의존도를 공개했습니다.
- 5C단계 완료 — 8개 기업의 `숫자와 비교 보기`를 연간·분기, 기간 추세·비교기업·산업 집계로 읽는 비교 중심 재무 피벗 분석판으로 분리했습니다.
- 5C.1단계 완료 — 최신 SEC·OpenDART 공시 식별자와 연결·독립 분기 기준을 감사하고, 연속 4분기 TTM·PER·PBR·PSR·외부 2곳 교차검증 규칙을 Production에 반영했습니다.
- 5I단계 완료 — SK하이닉스·Alphabet·하나금융지주의 기업 상세를 `기업 방향·시장 기대 → 네 가지 판단` 흐름으로 개편하고 공식 자료 최신성 게이트를 추가했습니다.
- 5단계 예정 — 거시경제 화면을 재구성합니다.
- 6단계 예정 — 기업과 거시경제의 연결을 강화합니다.
- 7단계 예정 — 시장 변곡점 연동을 다룹니다.

기업 검색은 현재 11개 프로필 registry에서 index를 파생합니다. 외부 검색 API, 실시간 주가 검색, 추천 점수와 검색 기록 저장은 사용하지 않습니다.
...
전체 관계도는 `MarketMapGraphShell`, `MarketMapGraphToolbar`, `MarketMapGraphLegend`를 네 지도에서 공유합니다. 닫힘/열림 CTA, 지역 필터, 선택 기업 중심·전체 맞춤, 680px desktop·560px mobile canvas, `padding 0.22`, `minZoom 0.42`, `maxZoom 1.28`, 고정 node, 텍스트 legend를 같은 위치와 크기로 사용합니다. AI 관계도는 전체 registry 행 번호가 아니라 현재 렌더되는 기업 집합 안에서 단계별 행을 다시 매겨 큰 빈 공간과 전체 맞춤 clipping을 막습니다. 760px 이하에서는 같은 기업 집합을 3열 compact graph 좌표로 바꿔 최소 zoom에서도 node·edge가 canvas 안에 들어오게 하고, 인프라 지도 좌표도 공통 canvas 밀도로 정규화합니다. 국가 필터는 산업 단계 노드는 유지하고 다른 국가 기업을 흐리게 표시합니다. TSMC는 대만, ASML은 네덜란드, Schneider Electric은 프랑스로 해석해 기타·글로벌로 분류하며 미국 상장 시장과 기업 소재 국가를 혼동하지 않습니다. node는 회사명 최대 2줄과 국가·역할 badge를 표시하고, edge는 수요 blue·공급 green·생산/설비 violet·시장 흐름 gray dashed 규칙을 사용합니다.
...
현재 11개 기업 상세는 검증된 Company Brief를 바탕으로 첫 판단을 제공합니다. 이 가운데 SK하이닉스·Alphabet·하나금융지주는 Phase 5I의 `기업 방향·시장 기대 → 본업 성장·이익의 질·현금의 질·투자·재무 부담` 흐름을 적용했고, 나머지 기업은 기존 다섯 질문 구조를 유지합니다. 첫 화면 지표는 기존 source가 있는 최대 3개로 제한하고, 비교가 없으면 0을 만들지 않습니다. NVIDIA와 Meta만 실제 심층 리포트 CTA를 표시하며 기존 재무·공시·차트·산업 연결은 결론 아래에서 유지합니다. 모델과 게시 계약은 `docs/company-brief-model.md`, `docs/company-judgment-model.md`, `docs/stock-dissection-intake.md`를 참고하세요.
...
NVIDIA와 Meta는 `/ko/companies/{slug}/valuation`에서 저장된 시장가격과 기준시각, 보수·기준·낙관 가치 범위, 범위 안 가격 위치, Reverse DCF의 7년 내재 매출 CAGR, 근거 상태가 붙은 기대 후보, WACC×영구성장률 민감도와 다음 확인 항목을 순서대로 설명합니다. 나머지 9개 기업은 검증 모형 부재를 0이나 빈 그래프로 대체하지 않습니다.

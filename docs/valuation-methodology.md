...
DCF·Reverse DCF·민감도와 확률적 범위의 완전한 표·계산 설명은 Valuation 화면을 단일 공개 위치로 사용한다. Research Report에는 같은 표를 복제하지 않는다. 계산 artifact, model version, 가정 변경 이력은 감사와 재현을 위해 기존 저장 위치에 보존한다.

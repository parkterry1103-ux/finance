...
인쇄는 브라우저 `window.print()`만 사용한다. `@page`는 A4, 여백은 위 16mm·좌우 14mm·아래 18mm다. 화면 내비게이션·목차·인쇄 버튼을 숨기고 흰 배경을 사용하며, 카드·표·증거 행의 불필요한 페이지 분할을 피한다.
...
최종 A4 PDF는 NVIDIA 11쪽, Meta 11쪽이다. 22쪽을 모두 PNG로 다시 렌더링해 빈 페이지, 잘린 문장·표·차트·민감도, 겹친 요소, 깨진 한글, 고립된 제목, 내비게이션·버튼 출력, 출처·방법론·주의 문구 누락이 없음을 확인했다.

...
export function StockSummaryCard({ item, onNavigate, compact = false }: { item: StockDissectionSummary; onNavigate: EditorialNavigate; compact?: boolean }) {
  ...
  const comparisons = [
    item.comparison?.market ? { label: `${item.comparison.market.name} 대비`, value: relativeReturn(item.priceMove.value, item.comparison.market.value) } : null,
    item.comparison?.sector ? { label: `${item.comparison.sector.name} 대비`, value: relativeReturn(item.priceMove.value, item.comparison.sector.value) } : null,
  ].filter((comparison): comparison is { label: string; value: number } => Boolean(comparison));
  return (
    <article className={`editorial-card editorial-stock-card${compact ? ' is-compact' : ''}`}>
      ...
      <div className="editorial-move-row">
        <strong aria-label={`등락률 ${formatSignedPercent(item.priceMove.value)}`}>{formatSignedPercent(item.priceMove.value)}</strong>
        {comparisons.map((comparison) => <span key={comparison.label}>{comparison.label} {formatRelativeReturn(comparison.value)}</span>)}
      </div>
      <h3>{item.headline}</h3>
      <dl className="editorial-card-facts">
        <div><dt>직접적인 계기</dt><dd>{item.directCatalyst}</dd></div>
        <div><dt>움직임의 성격</dt><dd>{moveCharacterLabels[item.moveCharacter]}</dd></div>
      ...
      <div className="editorial-card-actions">
        ...
        {item.company.companySlug ? <a href={`/ko/companies/${encodeURIComponent(item.company.companySlug)}`} onClick={editorialInternalLink(`/ko/companies/${encodeURIComponent(item.company.companySlug)}`, onNavigate)}>기업 분석 보기</a> : null}
      </div>
    </article>
  );
}
...
export function ThreeReadsSummaryCard({ item, onNavigate, compact = false }: { item: ThreeReadsSummary; onNavigate: EditorialNavigate; compact?: boolean }) {
  ...
  return (
    <article className={`editorial-card editorial-three-card${compact ? ' is-compact' : ''}`}>
      <div className="editorial-card-meta"><span>오늘의 3Reads</span><time dateTime={item.contentAsOf}>{editorialDate(item.contentAsOf)}</time></div>
      <h3>{item.centralQuestion}</h3>
      {!compact ? <p className="editorial-three-thread">{item.commonThread}</p> : null}
      <ol>{item.readHeadlines.map((headline) => <li key={headline}>{headline}</li>)}</ol>
      <div className="editorial-takeaway"><span>오늘의 한 줄</span><p>{item.oneLineTakeaway}</p></div>
      <div className="editorial-card-actions"><a href={path} onClick={editorialInternalLink(path, onNavigate)}>3Reads 전체 읽기 <ArrowRight size={15} aria-hidden="true" /></a></div>
    </article>
  );
}

...
import { MacroDomainSummaryCards } from './MacroDashboard.js';
...
function MacroIndicatorCard({ definition, series, loading }: {
  ...
  return (
    <article className="macro-indicator-card" id={`macro-indicator-${definition.id}`}>
      <div className="macro-indicator-topline"><span>{macroFrequencyLabels[definition.frequency]}</span><code>{definition.seriesId}</code></div>
      <h3>{definition.label}</h3>
      <div className="macro-indicator-value"><strong>{latestValue(definition, series)}</strong><span>{definition.displayUnit}</span></div>
      {definition.seriesId === 'T10Y2Y' && Number.isFinite(series.changes.currentBasisPoints) ? <p className="macro-spread-bp">현재 금리차 {signed(series.changes.currentBasisPoints!, 0, 'bp')}</p> : null}
      ...
      <div className="macro-change-list">
        {items.map((item) => <div key={item.label}><span>{item.label}</span><strong className={`macro-change-${changeTone(item.value)}`}>{Number.isFinite(item.value) ? `${item.value! > 0 ? '↑' : item.value! < 0 ? '↓' : '—'} ${signed(item.value!, item.digits, item.suffix)}` : '확인 중'}</strong></div>)}
      </div>
      ...
      <div className="macro-indicator-copy"><p>{definition.interpretation}</p><small>{definition.caution}</small></div>
      {source ? <a className="macro-source-link" href={source.url} target="_blank" rel="noopener noreferrer">FRED에서 확인 <ExternalLink size={14} /></a> : null}
    </article>
  );
}
...
export function MacroDashboard() {
  ...
  useEffect(() => {
    let active = true;
    fetchMacroIndicators().then((result) => { if (active) setResponse(result); });
    return () => { active = false; };
  }, []);
  ...
  return (
    <main className="macro-dashboard-main">
      <section className="macro-dashboard-intro"><p className="home-kicker">공식 거시 데이터 · FRED</p><h1>돈의 흐름과 경기</h1><p className="beginner-professional-name">금리·유동성·산업 수요 온도판</p><p>금리와 돈의 흐름이 산업 수요에 어떤 배경이 되는지 차근차근 봅니다.</p><div><Info size={16} /><span>지표마다 발표 주기가 다르며 최신 값은 같은 날짜의 데이터가 아닐 수 있습니다.</span></div></section>
      <section className="beginner-page-overview" aria-labelledby="macro-overview-title"><span>한눈에 보기</span><h2 id="macro-overview-title">네 가지 질문부터 확인하세요</h2><div><p><strong>금리는 높은가요?</strong> 만기별 금리와 <TermHelp termId="yield-spread" label="장단기 금리차" />를 함께 봅니다.</p><p><strong>돈을 구하기 쉬운가요?</strong> <TermHelp termId="financial-conditions" label="금융여건" />의 방향을 확인합니다.</p><p><strong>시장에 돈이 늘고 있나요?</strong> <TermHelp termId="liquidity" label="유동성" /> 지표의 주기를 나눠 봅니다.</p><p><strong>실제 수요가 움직이나요?</strong> <TermHelp termId="industrial-production" label="산업생산" />과 <TermHelp termId="capacity-utilization" label="가동률" />을 봅니다.</p></div></section>
      <a className="macro-relations-cta" href="/ko/market-relations"><span><strong>이 지표가 시장과 함께 움직였는지 보기</strong><small>금리·금융여건·산업생산을 대표 시장과 비교합니다.</small></span><ArrowRight size={17} aria-hidden="true" /></a>
      <section className="macro-dashboard-summary" aria-labelledby="macro-summary-title"><div className="macro-section-head"><span>편집 해설</span><h2 id="macro-summary-title">현재 거시 환경 요약</h2></div><MacroDomainSummaryCards /></section>
      ...
      {macroDomainOrder.map((domain) => <section className="macro-domain-section" key={domain} aria-labelledby={`macro-domain-${domain}`}><div className="macro-section-head"><span>{macroDomainLabels[domain]}</span><h2 id={`macro-domain-${domain}`}>{macroDomainLabels[domain]}</h2></div><div className={`macro-indicator-grid domain-${domain}`}>{macroIndicatorsByDomain(domain).map((definition) => <MacroIndicatorCard key={definition.id} definition={definition} series={byId.get(definition.id)} loading={loading} />)}</div>{domain === 'industry-infrastructure' ? <a className="macro-demand-supply-cta" href="/ko/demand-supply">공급 부족 상태와 함께 보기 <ArrowRight size={15} aria-hidden="true" /></a> : null}</section>)}
      <p className="macro-revision-note">표시 값은 FRED의 최신 공개 관측값이며 이후 수정될 수 있습니다.</p>
      <section className="macro-related-section" aria-labelledby="macro-related-reports-title"><div className="macro-section-head"><span>더 깊게 보기</span><h2 id="macro-related-reports-title">거시 흐름과 연결된 보고서</h2></div><div className="macro-related-grid">{reports.map((report) => report ? <a key={report.id} href={`/ko/reports/${encodeURIComponent(report.slug)}`}><span>{report.publisher}</span><strong>{report.titleKo}</strong><small>{report.publishedAt.split('-').join('.')}</small></a> : null)}</div></section>
      <section className="macro-related-section" aria-labelledby="macro-related-bottlenecks-title"><div className="macro-section-head"><span>구조적 배경</span><h2 id="macro-related-bottlenecks-title">함께 확인할 공급망 병목</h2></div><div className="macro-related-grid bottlenecks">{bottlenecks.map((entry) => entry ? <a key={entry.id} href={`/ko/bottlenecks/${encodeURIComponent(entry.slug)}`}><span>공급망 병목</span><strong>{entry.shortTitle}</strong><small>거시 지표는 배경 정보이며 원인을 단정하지 않습니다.</small></a> : null)}</div></section>
    </main>
  );
}

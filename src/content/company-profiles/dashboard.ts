...
const dashboardMetricsBySlug: Record<string, DashboardMetric[]> = {
  ...
  supermicro: [
    metric({
      ...
      unit: '십억 달러',
      currency: 'USD',
      period: 'FY2026 4분기 예비 업데이트',
      periodType: 'pointInTime',
      description: '회사가 분기 중 받은 것으로 발표한 신규 주문입니다. 일부는 확정 주문이 아닐 수 있고 취소·지연될 수 있어 매출과 동일하지 않습니다.',
      sourceIds: ['smci-ir-2026-q4-preliminary-update', 'smci-sec-2026-preliminary-8k'],
      updatedAt: '2026-07-21',
      qualityStatus: 'ok',
    }),
    metric({
      id: 'smci-fy2026-q3-revenue-growth',
      label: '분기 매출 성장률',
      value: 122.678,
      formattedValue: '+122.7%',
      unit: '%',
      period: 'FY2026 3분기',
      periodType: 'quarterly',
      comparison: { label: '전년 동기 대비', value: 122.678, formattedValue: '+122.7%', direction: 'up' },
      description: 'SEC 10-Q의 같은 분기 매출을 비교한 값입니다.',
      sourceIds: ['smci-sec-2026-q3-10q'],
      updatedAt: '2026-05-11',
      qualityStatus: 'ok',
    }),
    metric({
      id: 'smci-fy2026-q3-gross-margin',
      label: '매출총이익률',
      value: 9.945,
      formattedValue: '9.9%',
      unit: '%',
      period: 'FY2026 3분기',
      periodType: 'quarterly',
      comparison: { label: '전년 동기 대비', value: 0.375, formattedValue: '+0.4%p', direction: 'up' },
      description: '분기 매출총이익을 분기 매출로 나눈 실제 공시 기준 값입니다.',
      sourceIds: ['smci-sec-2026-q3-10q'],
      updatedAt: '2026-05-11',
      qualityStatus: 'ok',
    }),
    metric({
      id: 'smci-fy2026-q3-free-cash-flow',
      label: '잉여현금흐름',
      value: -6.695704,
      formattedValue: '-$6.696B',
      unit: '십억 달러',
      currency: 'USD',
      period: 'FY2026 3분기',
      periodType: 'quarterly',
      description: '9개월 누적 영업현금흐름과 설비투자에서 이전 6개월 누적 값을 차감해 산출한 분기 값입니다.',
      sourceIds: ['smci-sec-2026-q3-10q'],
      updatedAt: '2026-05-11',
      qualityStatus: 'ok',
    }),
    metric({
      id: 'smci-fy2025-per-2026-07-22',
      label: '최근 회계연도 PER',
      value: 18.1905,
      formattedValue: '18.19배',
      unit: '배',
      period: '2026-07-22 정규장 종가 · FY2025 희석 EPS',
      periodType: 'pointInTime',
      description: '최근 4분기 희석 EPS를 안전하게 구성할 수 없어 FY2025 희석 EPS $1.68을 사용한 공시 기반 fallback입니다.',
      sourceIds: ['smci-sec-2025-10k', 'smci-yahoo-price-2026-07-22'],
...
const assessmentsBySlug: Record<string, CompanyAssessment[]> = {
  ...
  supermicro: [
    assessment('growth', '성장성', 'needsReview', 'FY2026 3분기 매출은 전년 동기보다 122.7% 늘었고 4분기 신규 주문은 600억달러를 넘었지만 주문의 확정성과 출하 전환은 아직 검증 전입니다.', ['smci-fy2026-q3-revenue-growth', 'smci-2026-ai-orders'], ['smci-sec-2026-q3-10q', 'smci-ir-2026-q4-preliminary-update']),
    assessment('profitability', '수익성', 'needsReview', '3분기 실제 매출총이익률은 9.9%였고 4분기 15~17% 예비 추정은 최종 실적으로 확인해야 합니다.', ['smci-fy2026-q3-gross-margin'], ['smci-sec-2026-q3-10q', 'smci-ir-2026-q4-preliminary-update']),
    assessment('cashFlow', '현금창출력', 'needsReview', '3분기 잉여현금흐름은 -66.96억달러로 주문 확대와 재고·매입채무가 실제 현금 전환으로 이어지는지 점검이 필요합니다.', ['smci-fy2026-q3-free-cash-flow'], ['smci-sec-2026-q3-10q']),
    assessment('financialHealth', '재무건전성', 'needsReview', '대규모 주문 대응에는 운전자본이 필요하며 3분기 음의 잉여현금흐름과 재고·매입채무 추이를 함께 봐야 합니다.', ['smci-fy2026-q3-free-cash-flow', 'smci-2026-potential-financing'], ['smci-sec-2026-q3-10q']),
    assessment('valuation', '밸류에이션', 'insufficientData', '최근 회계연도 희석 EPS 기준 PER은 18.19배지만 자체 과거 범위와 검증 모형이 없어 상대 위치를 정하지 않습니다.', ['smci-fy2025-per-2026-07-22'], ['smci-sec-2025-10k', 'smci-yahoo-price-2026-07-22']),
  ],
  netflix: [

...
export type FinancialAuditCompany = {
  ...
  auditedPrice: { value: number; currency: 'KRW' | 'USD'; asOf: string; session: 'regularClose'; sourceId: string; sourceUrl: string };
...
export const external = (provider: string, value: number, definition: string, asOf: string, sourceUrl: string, matchStatus: ExternalMultipleCheck['matchStatus'], retrievedAtOverride?: string): ExternalMultipleCheck => ({
  provider,
  value,
  definition,
  asOf,
  ...
  retrievedAt: retrievedAtOverride ?? retrievedAt,
  sourceUrl,
  matchStatus,
});
